...
function getDeps(elmJson, elmVersion) {
  const elm = os.platform() === "win32" ? "AppData/Roaming/elm" : ".elm";
  const elmHome = process.env.ELM_HOME || path.join(os.homedir(), elm);

  info(`  |> gathering dependencies from ${elmHome}`);

  const deps = {};
  const dependencies = elmJson.dependencies.direct || elmJson.dependencies;
  Object.keys(dependencies).forEach(pkg => {
    const [name, constraint] = [pkg, dependencies[pkg]];
    const root = path.join(elmHome, elmVersion, "package", name);
    const semver = findLastPkgVersion(root, constraint);

    if (semver !== undefined) {
      const version = [semver.major, semver.minor, semver.patch].join(".");
      const pkgPath = path.join(root, version);
      deps[name] = {
        version,
        readme: readFile(path.join(pkgPath, "README.md") || ""),
        docs: readFile(path.join(pkgPath, "docs.json") || "[]")
      };
    }
  });
...
class DocServer {
  constructor(dir = ".") {
    ...
    if (this.type === "application") {
      warning("\nelm-doc-preview does not build applications, only packages.");
      warning("Please build your application before starting elm-doc-preview");
      warning("or you may miss some dependencies.\n");
    }
  ...
  setupFilesWatcher() {
    ...
    const watcher = sane(this.dir, { glob });
  ...
  updateDeps() {
    this.deps = getDeps(this.elmJson, this.elmVersion);
  }
